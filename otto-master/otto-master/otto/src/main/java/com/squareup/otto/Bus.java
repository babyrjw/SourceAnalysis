...
public class Bus {
  ...
  public void post(Object event) {
    ...
    //根据event的类型找出所有Subscribe的方法，即EventHandler,为每一个EventHandler生成一个
    //EventWithHandler对象，放入处理队列eventsToDispatch中，分别调用一遍，如果没有Subscribe该Event的方法，
    //则post一个DeadEvent。放入处理队列后，如果当前已有线程在循环处理该队列，则当前线程立即返回，
    //否则当前线程开始处理该队列，一直到处理队列为空才返回。这里在处理时如果有其他开始post一个Event
    //进处理队列里，也是由当前线程处理，其他线程则立即返回。
  ...
  protected void dispatchQueuedEvents() {
    ...
    // the events to be dispatched after the in-progress dispatch is complete.
      ...
      //多线程并发调用该函数时，只有第一个函数会执行该函数，其他函数立即返回。避免函数重入问题和事件处理乱序问题。
      //保证分发的事件只有在当前处理的事件处理完成后才开始处理
